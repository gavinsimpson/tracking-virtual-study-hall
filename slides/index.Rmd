...
```{r chl-a-setup, cache = FALSE, echo = TRUE}
library("gratia")
library("ggplot2")
library("dplyr")
```

```{r chl-a-load-data, echo = TRUE}
library("gamair")
...
```{r draw-chl-model, fig.height = 8, fig.width = 10, echo = TRUE}
...
```{r m1-appraise, fig.height = 6, fig.width = 10, echo = TRUE}
...
```{r m2-appraise, fig.height = 6, fig.width = 10, echo = TRUE}
...
```{r m2-draw, fig.height = 12, fig.width = 15, echo = TRUE}
...
```{r summarise-posterior, echo = TRUE}
library("ggdist")
fs |>
  group_by(.draw) |>
  summarise(chl_a = mean(.fitted)) |>
...
.small[
```{r richness-vcov-print, echo = FALSE, dependson=-1, echo = TRUE}
...
```{r soap-film-fv}
fv <- fitted_values(m2, data = pdata)
```
